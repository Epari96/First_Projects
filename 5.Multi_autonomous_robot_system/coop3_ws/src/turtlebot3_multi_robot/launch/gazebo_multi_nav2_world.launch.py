...
def generate_launch_description():
    ...
    robots = [
        {'name': 'tb1', 'x_pose': '-7.5', 'y_pose': '-6.5', 'z_pose': 0.01},
        {'name': 'tb2', 'x_pose': '-7.5', 'y_pose': '-7.5', 'z_pose': 0.01},
    ...
    for robot in robots:
        ...
        message = '{header: {frame_id: map}, pose: {pose: {position: {x: ' + str(spawn_x) + ', y: ' + str(spawn_y) + ', z: 0.1}, orientation: {x: 0.0, y: 0.0, z: 0.0, w: 1.0000000}}, }}'
    ...
    ######################
    # # 'ctrltower.py'를 실행하는 Node 추가
    # ctrltower_node = Node(
    #     package='turtlebot3_multi_robot',  # 'ctrltower.py'가 속한 패키지 이름
    #     executable='ctrltower',  # 실행 파일 이름 (확장자 .py는 생략)
    #     name='ctrltower',  # 노드 이름 (원하는 대로 설정 가능)
    #     output='screen',  # 출력 형식
    #     parameters=[{'use_sim_time': True}]  # 필요에 따라 매개변수 추가
    # )

    # # 런치 파일의 LaunchDescription 객체에 추가
    # ld.add_action(ctrltower_node)
