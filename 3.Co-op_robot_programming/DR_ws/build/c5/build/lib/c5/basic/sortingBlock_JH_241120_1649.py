...
ROBOT_ID = "dsr01"
ROBOT_MODEL = "m0609"
VEL_LOW, ACC_LOW = 20, 20
VEL_HIGH, ACC_HIGH = 60, 60
...
def main(args=None):
    ...
    pos5 = posx(450, 50, 150, 90, -180, 90)
    ...
    Pallet_Point.append(posx(600, 150, 90, 90, -180, 90))
    Pallet_Point.append(posx(550, 150, 90, 90, -180, 90))
    Pallet_Point.append(posx(500, 150, 90, 90, -180, 90))
    Pallet_Point.append(posx(600, 100, 90, 90, -180, 90))
    Pallet_Point.append(posx(550, 100, 90, 90, -180, 90))
    Pallet_Point.append(posx(500, 100, 90, 90, -180, 90))
    Pallet_Point.append(posx(600, 50, 90, 90, -180, 90))
    Pallet_Point.append(posx(550, 50, 90, 90, -180, 90))
    Pallet_Point.append(posx(500, 50, 90, 90, -180, 90))
    ...
    if rclpy.ok():
        ...
        for i in range(0,8):
            ...
            if block_type == 0:
                pos_goal = pos5
                pos_goal[1] += 50 * Pallet_Stacked[0]
                Pallet_Stacked[0] += 1
                movel(pos_goal, vel=VEL_HIGH, acc=ACC_HIGH, ref=DR_BASE)
            elif block_type == 1:
                pos_goal = pos5
                pos_goal[0] -= 50
                pos_goal[1] += 50 * Pallet_Stacked[1]
                Pallet_Stacked[1] += 1
                movel(pos_goal, vel=VEL_HIGH, acc=ACC_HIGH, ref=DR_BASE)
            else:
                pos_goal = pos5
                pos_goal[0] -= 100
                pos_goal[2] += 50 * Pallet_Stacked[2]
                Pallet_Stacked[2] += 1
                movel(pos_goal, vel=VEL_HIGH, acc=ACC_HIGH, ref=DR_BASE)
            ...
            task_compliance_ctrl(stx=[500, 500, 500, 100, 100, 100])
            set_desired_force(fd=[0, 0, -20, 0, 0, 0], dir=[0, 0, 1, 0, 0, 0], mod=DR_FC_MOD_REL)
            while not check_force_condition(DR_AXIS_Z, max=10): # or get_current_posx()[2]>60:
                pass
            release_compliance_ctrl()
            ungrip()
